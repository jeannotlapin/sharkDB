sharksDB.Routers.App = Backbone.Router.extend({
	...
	species: function(id) {
		sharksDB.Models.currentState.set({country: '', rfmo: '', species: id.replace(/_/g, ' ')});
	},
	...
	updateNav: function() {
		if (sharksDB.Models.currentState.get('country') != '') {
			...
			this.navigate('species/'+sharksDB.Models.currentState.get('species').replace(/ /g, '_'), {trigger: false});
		} else {
